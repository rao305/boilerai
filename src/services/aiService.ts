...
class AIService {
  ...
  private async checkHealth(): Promise<boolean> {
    try {
      ...
      const response = await window.fetch(`${this.baseUrl}/health`, {
  ...
  async sendMessage(message: string, userId: string = 'anonymous'): Promise<string> {
    try {
      ...
      const response = await window.fetch(`${this.baseUrl}/chat`, {
        method: 'POST',
        headers: {
      ...
      // Use AI to generate contextual error response
      if (error instanceof Error) {
        if (error.name === 'AbortError') {
          return await pureAIFallback.generateErrorResponse(message, 'general', 'Request timeout - query processing taking longer than expected');
        } else if (error.message.includes('Failed to fetch')) {
          return await pureAIFallback.generateOfflineResponse(message, 'BoilerAI bridge service');
        }
      }
  ...
  private async getIntelligentFallback(message: string): Promise<string> {
    ...
    try {
      ...
      // Use AI to generate contextual fallback response instead of hardcoded logic
      return await pureAIFallback.generateOfflineResponse(
        message, 
        'Both primary AI service and OpenAI fallback are currently unavailable'
      );
    }
  }
  ...
  async uploadTranscriptContext(userId: string, transcript: TranscriptData): Promise<boolean> {
    try {
      ...
      const response = await window.fetch(`${this.baseUrl}/transcript/upload`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async getUserContext(userId: string): Promise<{hasContext: boolean; contextKeys?: string[]}> {
    try {
      const response = await window.fetch(`${this.baseUrl}/users/${userId}/context`);
  ...
  async generatePersonalizedPlan(userId: string, studentProfile: any, preferences?: any): Promise<any> {
    try {
      ...
      const response = await window.fetch(`${this.baseUrl}/generate-plan`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async getStudentProfile(userId: string): Promise<any> {
    try {
      ...
      const response = await window.fetch(`${this.baseUrl}/profile/${userId}`);
  ...
  async getPersonalizedRecommendations(userId: string, context: any): Promise<any> {
    try {
      ...
      const response = await window.fetch(`${this.baseUrl}/recommendations`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },

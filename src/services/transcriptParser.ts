...
${truncatedText}`;
    ...
    try {
      const response = await window.fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`, {
        method: 'POST',
        headers: {
      ...
      // Clean up placeholder values and reject mock data
      if (jsonData.student_info.name.includes('[') || 
          ...
          jsonData.student_info.name === 'Alex Johnson' ||
          jsonData.student_info.name.includes('placeholder') ||
          jsonData.student_info.name.includes('Test') ||
          jsonData.student_info.name.length < 3) {
        throw new Error('AI returned placeholder or mock data instead of extracting real transcript information. Please ensure your transcript contains actual student information.');
      }

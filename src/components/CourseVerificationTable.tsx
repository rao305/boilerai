...
export const CourseVerificationTable: React.FC<CourseVerificationTableProps> = ({
  ...
  const getGradeBadge = (grade: string) => {
    if (!grade) return <Badge>In Progress</Badge>;
    ...
    return (
      <Badge variant="outline" className={`${getGradeColor(grade)} text-white border-0`}>
        {grade}
      </Badge>
    );
  };
      ...
      <tr key={course.id} className={isSelected ? 'bg-neutral-950/40' : ''}>
          ...
          {isEditing ? (
            ...
            <div>
              ...
              {course.repeatIndicator && (
                <Badge className="ml-1 text-xs">{course.repeatIndicator}</Badge>
              )}
            </div>
          )}
        </td>
        <td className="p-3 align-top">
          {isEditing ? (
            ...
            <div>
              <span className="text-neutral-200">{course.courseTitle}</span>
          ...
          <div>
            ...
            <div className="space-y-2 text-neutral-300">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-sm">{transcriptData.uploadDate.toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        </div>

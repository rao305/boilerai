...
python3 simple_main.py > ../../../ai-bridge.log 2>&1 &
AI_BRIDGE_PID=$!
cd ../../..
...
for i in {1..30}; do
    if curl -s http://localhost:5001/api/health > /dev/null 2>&1; then
        echo -e "${GREEN}✅ Backend ready on http://localhost:5001${NC}"
        break
    fi
    sleep 1
    if [ $i -eq 30 ]; then
...
for i in {1..30}; do
    if curl -s http://localhost:5002/health > /dev/null 2>&1; then
        echo -e "${GREEN}✅ AI Bridge Service ready on http://localhost:5002${NC}"
        break
    fi
    sleep 1
    if [ $i -eq 30 ]; then
...
echo -e "${BLUE}🤖 AI Bridge: http://localhost:5002${NC}"
echo ""

...
// This endpoint processes transcript data but NEVER stores it on the server
// All processing is ephemeral - data is processed and returned immediately
// No transcript content, student information, or grades are persisted in any database
// No logs contain actual transcript content - only metadata for debugging
...
router.post('/process-text',
  ...
  async (req, res) => {
    try {
      ...
      const result = await transcriptController.processTranscriptText(req.body, req.user?.id);
      res.json(result);
    } catch (error) {
...
router.get('/status/:jobId', async (req, res) => {
  try {
    const status = await transcriptController.getProcessingStatus(req.params.jobId, req.user?.id);
    res.json(status);
  } catch (error) {

...
const productionValidator = require('./config/production');
...
try {
  ...
  productionValidator.validateProduction();
  productionValidator.applyDefaults();
} catch (error) {
  ...
  try {
    ...
    if (testUser) {
      ...
      testUser = new User({
        ...
        // FERPA COMPLIANCE: No educational data stored
        profileCompleted: false,
        lastTranscriptProcessed: null,
        preferences: {
          theme: 'system',
          notifications: true
        },
...
// CRITICAL SECURITY: Enhanced security headers for production
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      formAction: ["'self'"],
      frameAncestors: ["'none'"],
      upgradeInsecureRequests: configManager.isProduction() ? [] : null
    },
  },
...
// Very strict rate limiting for transcript processing
const transcriptLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 20, // 20 transcript uploads per hour
  message: {
    error: 'Transcript processing limit exceeded, please try again later',
    retryAfter: 3600
  },
  standardHeaders: true,
  legacyHeaders: false
